...
if($display_cfo_services && $cfo_services_select_post_category){
	$query=new WP_Query(['post_type'=>'post', 'posts_per_page'=>-1, 'category__in'   => [$cfo_services_select_post_category], 'date_query'     => $date_query]);
	if($query->have_posts()){
?>
...
if($display_categories_section && $categories_section_categories){
    ...
    if($select_category){
        $query=new WP_Query(['post_type'=>'post', 'posts_per_page'=>-1, 'category__in'   => [$select_category], 'date_query'     => $date_query]);
	    if($query->have_posts()){
?>
    <!-- ----------------------------------------------------------------------------------------------------------- -->
